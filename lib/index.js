module.exports = function (config) {
	...
	return {
      ...
      parserOpts.plugins.push('decorators', 'flow')
    },
		visitor: {
			CallExpression(path) {
				let node = path.node
			...
			Class(path) {
				path.get('body').get('body').forEach(entry => {
					if (!entry.node.decorators || !entry.get('decorators').forEach) {
						return
					}
					entry.get('decorators').forEach(path => {
						let decoratorExpression = path.node.expression
						if (decoratorExpression.name !== 'reactive') {
							return
						}
						let typeToExpression = (type) => {
							switch (type.type) {
								case 'GenericTypeAnnotation':
									return type.id
								case 'ObjectTypeAnnotation':
									return t.objectExpression(type.properties.map(property =>
										t.objectProperty(property.key, typeToExpression(property.value))))
								case 'ArrayTypeAnnotation':
									return t.arrayExpression([typeToExpression(type.elementType)])
						...
						let type = typeToExpression(path.parent.typeAnnotation.typeAnnotation)
						let name = path.parent.key

						path.parentPath.replaceWithMultiple([
							// get name() { return ... }
							t.classMethod('get', name, [], t.blockStatement([t.returnStatement(
									t.logicalExpression('||',
										t.logicalExpression('&&',
											t.memberExpression(t.thisExpression(), t.identifier('_properties')),
											t.memberExpression(t.memberExpression(t.thisExpression(), t.identifier('_properties')), name)),
										t.callExpression(t.memberExpression(decoratorExpression, t.identifier('get')),
											[t.thisExpression(), t.stringLiteral(name.name), type])))
								])),
							t.classMethod('set', name, [t.identifier('value')], t.blockStatement([t.expressionStatement(
									t.callExpression(t.memberExpression(decoratorExpression, t.identifier('set')),
										[t.thisExpression(), t.stringLiteral(name.name), t.identifier('value')])
								)]))])
					})
