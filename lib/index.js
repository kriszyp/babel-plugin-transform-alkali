  ...
  const reactVisitors = {
    AssignmentExpression(path) {
      ...
      if (t.isMemberExpression(path.node.left)) {
        let assignee = retrieveReactively(left.get('object'))
        path.replaceWith(markAsOutput(t.callExpression(
          t.memberExpression(
            t.conditionalExpression(
              t.logicalExpression('&&', assignee, t.memberExpression(assignee, t.identifier('set'))),
              assignee,
              t.assignmentExpression('=', assignee, t.callExpression(t.memberExpression(t.identifier('react'), t.identifier('from')), [assignee]))
            ),
            t.identifier('set')
          ), [t.stringLiteral(path.node.left.property.name), retrieveReactively(right)])))
        return
      }
      path.replaceWith(markAsOutput(t.callExpression(
        markAsOutput(t.memberExpression(retrieveReactively(left), t.identifier('put')),
        [retrieveReactively(right)]))))
    },
    MemberExpression(path) {
      let { node } = path
